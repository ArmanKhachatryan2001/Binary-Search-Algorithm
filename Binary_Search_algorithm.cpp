template<typename T>
int BS(T* arr, int size, T value) // binary search array
{
    int zero = 0;
    int new_size = size;
    int half = size / 2;
...
IteratorType search(IteratorType start, IteratorType end, T value)
{
	return BS(start, end,value, typename std::iterator_traits<IteratorType>::iterator_category());
}
